...
#  print "Placeholders set"
  ...
  f_ = D.inference(z_, phase_train) # reconstructed sample logits
...
#  disc_loss = -tf.reduce_mean(tf.log(prob_t_) + tf.log(1 - prob_f_))
  gener_loss = -tf.reduce_mean(tf.log(prob_f_))
...
#  disc_false_loss = -tf.reduce_mean(tf.log(1 - prob_f_))
  LOSS = LOSS
  genr_loss  = (gener_loss) + (10.0*LOSS) 
...
#    print "Training probabilities"
    ...
    prob_t, prob_f, pos_l,neg_l,total_disc_loss,generator_loss,adv_loss,con_loss,tl,fl,z = D.sess.run(
        [prob_t_,prob_f_,pos_loss,neg_loss,disc_loss,genr_loss,gener_loss,LOSS,t_,f_,z_],
        feed_dict={
          x_:X_train_batch,
          y_:Y_train_batch,
          phase_train:False

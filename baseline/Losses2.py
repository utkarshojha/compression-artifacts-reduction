...
def tv_loss(y_true,y_pred):
  ...
  loss = tf.reduce_sum(total_variation(y_pred))
  return loss



def perceptual_loss(y_true,y_pred, layer_list=['pool5'], alphas=[1.0]):
  ...
  def vgg_16(image, weights, biases, keep_prob=1.0):
    activations = {}
    ...
    with tf.name_scope("conv3"):
      ...
      activations['pool3'] = max_pool(activations['conv3_3'], k=2, s=2)
    ...
    with tf.name_scope("conv4"):
      ...
      activations['pool4'] = max_pool(activations['conv4_3'], k=2, s=2)
    ...
    with tf.name_scope("conv5"):
      ...
      activations['pool5'] = max_pool(activations['conv5_3'], k=2, s=2)
    return activations

  def get_weights():
    net = numpy.load('../Resources/VGG/vgg16.npy').item()
    weights = {}
    biases  = {}
  ...
  y_true_activations = vgg_16(y_true,weights,biases)
  y_pred_activations = vgg_16(y_pred,weights,biases)
  ...
  for layer_name,alpha in zip(layer_list,alphas):
    y = y_true_activations[layer_name]
    x = y_pred_activations[layer_name]
